...
function getInitialRoundsFromState(state: any): IRoundProps[] {
  if (state && state.groups && state.thirdPlaceTeams) {
    const matches = generateRoundOf16Matches(state.groups, state.thirdPlaceTeams);
    return [
      {
        title: 'Round of 16',
        seeds: matches.map((match, i) => ({
          id: i + 1,
          date: '2024-01-03',
          teams: [
            { name: `${match.homeTeam.flag} ${match.homeTeam.name}` },
            { name: `${match.awayTeam.flag} ${match.awayTeam.name}` },
          ],
        })),
      },
      {
  ...
  return [
    {
      ...
      seeds: Array.from({ length: 8 }, (_, i) => ({
        id: i + 1,
        date: '2024-01-03',
        teams: [demoTeams[i * 2], demoTeams[i * 2 + 1]],
      })),
    },
    {
      title: 'Quarter Finals',
...
const BracketPage: React.FC = () => {
  ...
  const [rounds, setRounds] = useState<IRoundProps[]>(getInitialRoundsFromState(location.state));
  ...
  return (
    <>
      <div style={{ width: '100%', display: 'flex', justifyContent: 'center', margin: '16px 0', gap: 16 }}>
        <button
          style={{
            padding: '8px 20px',
            borderRadius: 6,
        ...
        <button
          ...
          onClick={() => {
            setRounds(getInitialRoundsFromState(location.state));
            setChampion('');
          }}
        >
          Reset Bracket
